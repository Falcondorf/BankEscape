...
drop primary key;
alter table SCORE
drop foreign key fkScoreP;
alter table SCORE
drop foreign key fkScoreL;
...
create table SCORE (
    scoreId numeric(10) not null,
    pId numeric(10) not null,
    lId numeric(10) not null,
    score numeric(10) not null,
        constraint IDSCORE primary key(scoreId)
);
-- Constraints Section
-- ___________________ 
...
alter table SCORE add constraint fkScoreP
    foreign key (pId) references PLAYER(pId);

alter table SCORE add constraint fkScoreL
...
Insert Into SEQUENCES Values('SCORE', 0);
